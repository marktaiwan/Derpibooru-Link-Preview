...
(function () {
    ...
    const config = ConfigManager('Ponybooru Comment Enhancements', 'pony_comment_enhancements');
    ...
    */
    ...
    function getImageId(url) {
        const regex = new RegExp('https?://(?:www\\.)?(?:(?:ponybooru\\.org)/(?:images/)?(\\d+)(?:\\?.*|/|\\.html)?|ponybooru\\.org/img/(?:view/|download/)?\\d+/\\d+/\\d+/(\\d+))', 'i');
        ...
        links.forEach((link) => {
            ...
            if (DISABLE_NATIVE_PREVIEW && !isForumPost) {
                link.addEventListener('click', (e) => {
                    ...
                    // quoted links doesn't contain query strings, this prevents page reload on links like "ponybooru.org/1234?q=tag"
    ...
    if (LINK_IMAGES) {
        NodeCreationObserver.onCreation('.communication__body__text .imgspoiler>img, .image-description .imgspoiler>img', img => {
            ...
            if (imageId !== null) {
                // image is on Ponybooru
                anchor.href = `/${imageId}`;
            } else {
                // camocdn.ponybooru.org/
