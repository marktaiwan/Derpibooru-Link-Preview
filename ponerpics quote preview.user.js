...
(function () {
    ...
    const config = ConfigManager('Ponybooru Comment Enhancements', 'poner_comment_enhancements');
    ...
    */
    ...
    function getImageId(url) {
        const regex = new RegExp('https?://(?:www\\.)?(?:(?:ponerpics\\.org)/(?:images/)?(\\d+)(?:\\?.*|/|\\.html)?|ponerpics\\.org/img/(?:view/|download/)?\\d+/\\d+/\\d+/(\\d+))', 'i');
        ...
        links.forEach((link) => {
            ...
            if (DISABLE_NATIVE_PREVIEW && !isForumPost) {
                link.addEventListener('click', (e) => {
                    ...
                    // quoted links doesn't contain query strings, this prevents page reload on links like "ponerpics.org/1234?q=tag"
